...
const ProductForm = ({ editForm = false, initialData = null }: ProductFormProps) => {
    const validationSchema = Yup.object().shape({
        ...
        main_image_id: editForm ? Yup.number().nullable() : Yup.number()
    ...
    const transformAvailabilityStatus = (status: string) => {
    ...
    useEffect(() => {
        getFormInitialApi();
        if (initialData) {
            formik.setValues({ ...initialData, main_image_id: null, gallery_image_ids: [], category_ids: initialData.categories.map((cat: any) => cat.id) });
            setMainImage({ ...initialData.image, thumbnail: initialData.image?.thumb });
            setGalleryImages(initialData.gallery?.map((image: any) => ({ ...image, thumbnail: image?.thumb })) || []);
            setSelectState({
                categories: initialData.categories.map((cat: any) => ({ value: cat.id, label: cat.name })),
                author: initialData.author ? { value: initialData.author.id, label: initialData.author.name } : null,
                publisher: initialData.publisher ? { value: initialData.publisher.id, label: initialData.publisher.name } : null,
                type: initialData.type ? { value: initialData.type, label: transformAvailabilityStatus(initialData.type) } : null,
                language: initialData.language ? { value: initialData.language.id, label: initialData.language.name } : null,
                availability: initialData.status ? { value: initialData.status, label: transformAvailabilityStatus(initialData.status) } : null,
                tags: initialData.tags.map((tag: string) => ({ value: tag, label: tag })),
            });
        }
    }, [initialData])
    ...
    const formik = useFormik({
        ...
        onSubmit: async (value, { resetForm }) => {
            ...
            try {
                let res;
                if (editForm && initialData) {
                    res = await api.put(`/products/${initialData.id}`, value);

                } else {
                    res = await api.post('/products', value);
    ...
    return (
        <Fragment>
            <Seo title={editForm ? "Edit Products" : "Add Products"} />
            <Pageheader currentpage={editForm ? "Edit Products" : "Add Products"} activepage="Products" mainpage={editForm ? "Edit Products" : "Add Products"} />
                    ...
                    <div className="box">
                        <div className="box-body add-products !p-0">
                                        ...
                                        <div className="grid grid-cols-12 gap-4">
                                            ...
                                            <div className="xl:col-span-12 col-span-12">
                                                <div className="flex items-center">
                                                    <input type="checkbox" id="hs-basic-with-description-unchecked" className="ti-switch mb-4" name='is_featured' checked={formik.values.is_featured} onChange={() => formik.setFieldValue('is_featured', !formik.values.is_featured)} />
                            ...
                            <div className="px-6 py-4 border-t border-dashed dark:border-defaultborder/10 sm:flex justify-end gap-3">
                                <Button disabled={isLoading} loading={isLoading} type="button" color='primary' variant='light' onClick={addProduct}>{editForm ? 'Update' : 'Add'} Product</Button>
